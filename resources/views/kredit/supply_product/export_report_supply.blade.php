		...
		<ul>
			...
			<table class="w-100 mb-4">
				...
				<tbody>
					...
					<tr>
						<td class="text-left text-12 txt-dark">{{ date('H:i', strtotime($sp->created_at)) }}</td>
						<td>
							<span class="text-left d-block text-12 txt-dark2">{{ $sp->product->nama_barang }}</span>
							<span class="text-left d-block text-10 txt-light">{{ $sp->product->kode_barang }}</span>
						</td>
						<td class="text-left text-12 txt-dark2">{{ $sp->jumlah }}</td>
						<td class="text-left text-12 txt-dark">Rp. {{ number_format($sp->harga_beli,2,',','.') }}</td>
						@php
						$total = $sp->jumlah * $sp->harga_beli;
						$pengeluaran += $total;
						@endphp
						<td class="text-left text-12 txt-green">Rp. {{ number_format($total,2,',','.') }}</td>
						<td class="text-left text-12">{{ $sp->supply->supplier->name }}</td>
					</tr>
					@endforeach
				</tbody>
			</table>
